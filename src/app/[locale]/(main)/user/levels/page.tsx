...
export default function RewardsPage() {
  ...
  const [previewCompletedRentals, setPreviewCompletedRentals] = useState<number>(session?.user?.rentsCount || 0);
  ...
  useEffect(() => {
    ...
    const currentLevel = levels?.find((l) => l._id === session?.user?.level);
  ...
  }, [carouselApi, previewCompletedRentals, levels]);
  ...
  const currentLevelData = levels.find((l) => l._id === session?.user?.level) || levels[0];
  ...
  return (
    <div className="space-y-6 pb-8">
        ...
        <div className="relative px-6 py-8 lg:px-10 lg:py-12">
          ...
          <div>
            ...
            <Typography variant="h4" as="h4" className="mt-4 text-white font-semibold">
              {t("hero.complete-message", { nextLevel: Math.min(currentLevelData.level + 1, 5) })}
            </Typography>
          </div>
          ...
          {currentLevelData.rents > 10 ? (
            ...
            <div className="mx-auto mt-4 w-full max-w-md px-12 lg:max-w-lg lg:px-16">
              <Carousel
                key={`carousel-level-${session?.user?.level}`}
                opts={{
                  align: "start",
                  loop: false,
                  dragFree: true,
          ...
          <Typography variant="body-sm" as="p" className="mt-4 text-center text-white hover:underline">
      ...
      <div id="levels" className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm lg:p-8">
        ...
        >
          {levels.map((level) => {
            const isCurrentLevel = currentLevelData.level === level.level;
