...
export function InstallmentRequestCard({
  ...
  const hasModal =
    request.ownerStatus?.toLowerCase() === "approved" &&
    request.userStatus?.toLowerCase() === "pending";


  return (
    <div
      onClick={() => onCardClick(request.id)}
      className="rounded-lg border bg-white p-6 hover:shadow-md cursor-pointer transition-shadow border-gray-200"
    >
      <div className={cn("flex items-start justify-between mb-4", locale === "ar" && "flex-row-reverse")}>
        <div className={cn("flex items-start gap-3", locale === "ar" && "flex-row-reverse")}>
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-main-100 shrink-0">
            <DollarSign className="h-6 w-6 text-main-600" />
          </div>
